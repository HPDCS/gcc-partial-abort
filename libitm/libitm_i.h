...
/* Extended jmpbuf */
typedef struct _ext_jmpbuf {
	// This is the space for general purpose registers
	unsigned long long rax;		//8 bytes - 0 -> 7
	unsigned long long rdx;		//8 bytes - 8 -> 15
	unsigned long long rcx;		//8 bytes - 16 -> 23
	unsigned long long rbx;		//8 bytes - 24 -> 31
	unsigned long long rsp;		//8 bytes - 32 -> 39
	unsigned long long rbp;		//8 bytes - 40 -> 37
	unsigned long long rsi;		//8 bytes - 48 -> 55
	unsigned long long rdi;		//8 bytes - 56 -> 63
	unsigned long long r8; 		//8 bytes - 64 -> 71
	unsigned long long r9; 		//8 bytes - 72 -> 79
	unsigned long long r10;		//8 bytes - 80 -> 87
	unsigned long long r11;		//8 bytes - 88 -> 95
	unsigned long long r12;		//8 bytes - 96 -> 103
	unsigned long long r13;		//8 bytes - 104 -> 111
	unsigned long long r14;		//8 bytes - 112 -> 119
	unsigned long long r15;		//8 bytes - 120 -> 127
	unsigned long long rip;		//8 bytes - 128 -> 135
	unsigned long long flags;	//8 bytes - 136 -> 143

	// Space for FPU registers
	unsigned char fpu[512] __attribute__((aligned(16))); // fxsave wants 16-byte aligned memory
	//512 bytes - 144 -> 656
	// Had to be at least 144, since it is aligned to 16, offset is 144 
} __attribute__((aligned(16))) ext_jmpbuf; //Just to be sure
...
struct gtm_thread{
  ...
  // Implemented in beginend.cc
    void rollback (gtm_transaction_cp *cp = 0, bool aborting = false, int r_i = 0, int w_i = 0);
    bool trycommit ();
    /* void restart (gtm_restart_reason, bool finish_serial_upgrade = false)
          ITM_NORETURN; */
    void restart (gtm_restart_reason, bool finish_serial_upgrade = false, int re = 0, int we = 0)
          ITM_NORETURN;

    gtm_thread();   // Costruttore senza parametri
    ~gtm_thread();  // Distruttore
...
extern abi_dispatch *dispatch_ml_wt_pr();
