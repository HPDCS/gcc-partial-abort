...
/* The multiple lock, write-through TM method.
   ...
   transactions. (Ho un lock per ogni zona, se una transazione locka A e deve lockare B, e una locka B e deve lockare A -> DEADLOCK)
...
class ml_wt_dispatch : public abi_dispatch {
  protected:
    ...
    // Returns true iff all the orecs in our read log still have the same time or have been locked by the transaction itself.
    static bool validate(gtm_thread *tx){
      ...
      // most recent orec updates before the global time that we load.
        gtm_word o = i->orec->load(memory_order_relaxed);
          ...
          // number bits.
        if (ml_mg::get_time(o) != ml_mg::get_time(i->value) && o != locked_by_tx)
            return false;
      }
    ...
    static gtm_rwlog_entry* pre_load(gtm_thread *tx, const void* addr, size_t len){
      ...
      do{
          ...
          if (likely (!ml_mg::is_more_recent_or_locked(o, snapshot))){
              ...
              // If the orec is locked by us, just skip it because we can just read from it.  Otherwise, restart the transaction.
              if (o != locked_by_tx)
    ...
    // Restart the transaction if any of the orecs is locked by another transaction.
  ...
  public:
    ...
    virtual bool trycommit(gtm_word& priv_time){
      ...
      va_list list;
      fputs ("\n_____: ", stdout);
      vfprintf (stdout, "committing method-ml", list);
      fputc ('\n', stdout);
...
abi_dispatch* GTM::dispatch_ml_wt (){
