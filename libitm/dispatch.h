...
public:
  ...
  // Rolls back a transaction. Called on abort or after trycommit() returned false.
  virtual void rollback(gtm_transaction_cp *cp = 0) = 0;
  
  // TODO: write what it does
  virtual void partial_rollback(gtm_transaction_cp *cp = 0, int r_i = 0, int w_i = 0) = 0;
  ...
  // Returns STATE_SERIAL or STATE_SERIAL | STATE_IRREVOCABLE iff the TM method only works for serial-mode transactions.
...
public:
  static bool memmove_overlap_check(void *dst, const void *src, size_t size, ls_modifier dst_mod, ls_modifier src_mod);
