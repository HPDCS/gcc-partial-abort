...
GTM::gtm_thread::rollback (gtm_transaction_cp *cp, bool aborting, int r_i, int w_i)
{
...
GTM::gtm_thread::restart (gtm_restart_reason r, bool finish_serial_upgrade, int re, int we)
{
  ...
  if (re == 0 and we == 0) {
	    size_t i = 0;
	    printf("%p: trying partial rollback\n", tx);
	    fflush(stdout);
        for (i = 0; i < tx->readlog.size(); i++) {
			...
			if (o & ((~(gtm_word)0 >> 1) + 1)){
				...
				// (o != locked_by_tx)
				if (o != (((uintptr_t)tx >> 1) | ((~(gtm_word)0 >> 1) + 1))) {
					...
					/* Check timestamp */
					// (ml_mg::get_time(o) != ml_mg::get_time(tx->readlog[i].value))
					printf("%p: restart get_time(o)= %ld, get_time(i->value)= %ld\n", tx, o >> 3, tx->readlog[i].value);
					fflush(stdout);
					if (o >> 3 != tx->readlog[i].value >> 3){
						printf("%p: o != log\n", tx);
						fflush(stdout);
						break;
					}
  ...
  if (re != 0) {
	...
	/* remove comment to make the dispatcher restore the partial state */
	//jb = tx->readlog[re].jb;
  }
...
_ITM_commitTransaction(void)
{
  ...
  gtm_thread *tx = gtm_thr();
  if (!tx->trycommit ()){
	 printf("%p: calling rollback from commit transaction\n", tx);
	 fflush(stdout);
...
_ITM_commitTransactionEH(void *exc_ptr)
{
  ...
  if (!tx->trycommit ())
    {
		printf("%p: calling rollback from commit transaction EH!\n", tx);
	 fflush(stdout);
