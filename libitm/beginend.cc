...
// Vedere se funziona
__attribute__ ((__noreturn__)) __attribute__ ((noinline)) 
void _ext_longjmp(ext_jmpbuf *jb, long long val){
	asm volatile(\
		"movq %rdi, %rax											\n"\
																	   \
		...
		"movq " DCHPC_STR(offsetof_ext_jmpbuf_r15) "(%rax), %r15	\n"\
																	   \
		"frstor " DCHPC_STR(offsetof_ext_jmpbuf_fpu) "(%rax)		\n"\
...
#define ext_longjmp(jb_ptr, val)	_ext_longjmp(jb_ptr, val)

#pragma GCC poison _ext_longjmp
...
void* GTM::gtm_thread::operator new (size_t s){
  void *tx;
...
/* Free the given transaction. Raises an error if the transaction is still in use. */
void GTM::gtm_thread::operator delete(void *tx){
  free(tx);
}
...
uint32_t GTM::gtm_thread::begin_transaction (uint32_t prop, const gtm_jmpbuf *jb){

  printf("JB->RIP: %llx \t go check in the disasm code\n", jb->rip); fflush(stdout);
  ...
  tx = gtm_thr();
  if (unlikely(tx == NULL)){
      // Create the thread object. The constructor will also set up automatic deletion on thread termination.
      tx = new gtm_thread();
      set_gtm_thr(tx);
  }
  ...
  if (tx->nesting > 0){
	  ...
	  // We can use flat nesting, so elide this transaction.
	      if (!(prop & pr_instrumentedCode)){
	        if (!(tx->state & STATE_SERIAL) || !(tx->state & STATE_IRREVOCABLE))
		        tx->serialirr_mode();
        }
  ...
  // As long as we have not exhausted a previously allocated block of TIDs, we can avoid an atomic operation on a shared cacheline.
  ...
  // Log the number of uncaught exceptions if we might have to roll back this state.
  ...
  // Determine the code path to run. Only irrevocable transactions cannot be restarted, so all other transactions need to save live variables.
...
void GTM::gtm_transaction_cp::save(gtm_thread* tx){
...
void GTM::gtm_transaction_cp::commit(gtm_thread* tx){
...
void GTM::gtm_thread::rollback (gtm_transaction_cp *cp, bool aborting, int r_i, int w_i){
  ...
  if (r_i == 0) {
    ...
    // If vectr allocates contiguously 
    nb_writes = (int) (tx->writelog.end() - tx->readlog[r_i].next_write_addr) / sizeof(gtm_rwlog_entry);
    
    undolog.rollback (this, tx->writelog.size() - nb_writes);
    
    abi_disp()->partial_rollback (cp, r_i, tx->writelog.size() - nb_writes);
    */

    // Rollback parziale
    undolog.rollback (this, w_i);
    abi_disp()->partial_rollback (cp, r_i, w_i);
  }
...
void ITM_REGPARM _ITM_abortTransaction (_ITM_abortReason reason){
...
bool GTM::gtm_thread::trycommit (){
  ...
  if (abi_disp()->trycommit (priv_time)){
      ...
      if (state & gtm_thread::STATE_SERIAL){
          ...
          if (priv_time){
            do_read_unlock = true;
            shared_state.store((~(typeof gtm_thread::shared_state)0) - 1, memory_order_release);
          }
          else
...
void ITM_NORETURN GTM::gtm_thread::restart(gtm_restart_reason r, bool finish_serial_upgrade, int re, int we){
  ...
  if (re == 0 and we == 0) {
      ...
      for (i = 0; i < tx->readlog.size(); i++) {
        ...
        // (ml_mg::is_locked(o))
        if (o & ((~(gtm_word)0 >> 1) + 1)){ // if(is_locked(o))
          /* Check if orec is locked by tx */
          // (o != locked_by_tx)
          if (o != (((uintptr_t)tx >> 1) | ((~(gtm_word)0 >> 1) + 1))) { // if (o != locked_by_tx)
            /* orec is locked by another tx, so read is invalid */
            printf("%p: orec is locked by another tx, so read is invalid\n", tx);
            fflush(stdout);
            break;
          }
          else {
            /* orec is locked by me */
            /* Check timestamp */
            printf("%p: RESTART: get_time(o)= %ld, get_time(i->value)= %ld\n", tx, o >> 3, tx->readlog[i].value);
            fflush(stdout);
            if (o >> 3 != tx->readlog[i].value >> 3){ // if(ml_mg::get_time(o) != ml_mg::get_time(tx->readlog[i].value))
              break;
            }
          }
        }
		  }
      printf("%p: first vnr: %ld, rlsize: %ld\n", tx, i, tx->readlog.size());
      fflush(stdout);
		
      if (i < tx->readlog.size()) {
        size_t first_non_valid_read_index = i;
        
        size_t first_non_valid_write_index = tx->readlog[first_non_valid_read_index].next_write_index;
        
        printf("%p: first non-valid read: %ld, first non-valid write: %ld\n", tx, first_non_valid_read_index, first_non_valid_write_index);
        fflush(stdout);
        ...
        //printf("%p: first non valid: %p, last rs entry: %p\n", tx, first_non_valid, last_rs_entry);
        //fflush(stdout);
        
        re = (int)first_non_valid_read_index;
        we = (int)first_non_valid_write_index;
  ...
  /*-------------------------*/
  printf("%p: calling rollback with re: %d, we: %d\n", tx, re, we);
  fflush(stdout);
  rollback (0, false, re, we);
  ...
  /* If a partial rollback is needed, use jump buffer of read entry */
  if (re != 0) {
    printf("%p: preparing jb\n", tx);
    fflush(stdout);
    
    ext_jmpbuf ljb = tx->readlog[re].jb;
  ...
  printf("%p: jumping... \n", tx);
  fflush(stdout);
  GTM_longjmp (choose_code_path(prop, disp) | a_restoreLiveVariables, &jb, prop);
}

void ITM_REGPARM _ITM_commitTransaction(void){
  ...
  gtm_thread *tx = gtm_thr();
  if (!tx->trycommit ()){
	 printf("%p: calling rollback from _ITM_commitTransaction\n", tx);
	 fflush(stdout);
    tx->restart (RESTART_VALIDATE_COMMIT);
	}
}

void ITM_REGPARM _ITM_commitTransactionEH(void *exc_ptr){
  #if defined(USE_HTM_FASTPATH)
  ...
  gtm_thread *tx = gtm_thr();
  if (!tx->trycommit ()){
		printf("%p: calling rollback from _ITM_commitTransactionEH\n", tx);
	  fflush(stdout);
