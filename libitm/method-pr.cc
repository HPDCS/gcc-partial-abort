...
{
  static const gtm_word LOCK_BIT = (~(gtm_word)0 >> 1) + 1; // 1000 0000
  ...
  static bool is_locked(gtm_word o) { return o & LOCK_BIT; } // 1000 0000 if locked, 0000 0000 if not locked ?
  
  static gtm_word set_locked(gtm_thread *tx) {
    return ((uintptr_t)tx >> 1) | LOCK_BIT; // xxxxxxx OR 1000 0000 = 1xxx xxxx ?
  }
...
protected:
    ...
    do
      {
        ...
        if (likely (locked_by_tx != o)) {
			/* tx did not acquired this specific o ???*/
            ...
            if (unlikely ((ml_mg::is_locked(o)) && 0)){
				printf("%p: 190\n", tx);
				fflush(stdout);
            ...
            if (unlikely (ml_mg::get_time(o) > snapshot)) {
					//printf("%p: (ml_mg::get_time(o) > snapshot)\n", tx);
					//fflush(stdout);
  ...
  static bool validate(gtm_thread *tx) {
	...
	// most recent orec updates before the global time that we load.
        ...
        // number bits.
        printf("%p: validate: get_time(o)= %ld, get_time(i->value)= %ld\n", tx, ml_mg::get_time(o), ml_mg::get_time(i->value));
		fflush(stdout);
    ...
    do
      {
        ...
        if (likely (!ml_mg::is_more_recent_or_locked(o, snapshot))) {
			// snapshot > o
				//printf("%p: o is more recent! o: %d, snapshot %d\n", tx, (unsigned int)ml_mg::get_time(o), (unsigned int)snapshot);
				//fflush(stdout);
            success:
            gtm_rwlog_entry *e = tx->readlog.push();
            e->orec = o_ml_mg.orecs + oi.get();
            e->value = o;
            /* New entry has first free entry reference of writelog */
            e->next_write_index = tx->writelog.size();
			...
			asm volatile("movq %%rbx, %0": "=m"(e->jb.rbx));
			asm volatile("movq %%rbp, %0": "=m"(e->jb.rbp));
			asm volatile("movq %%r12, %0": "=m"(e->jb.r12));
			asm volatile("movq %%r13, %0": "=m"(e->jb.r13));
			asm volatile("movq %%r14, %0": "=m"(e->jb.r14));
			asm volatile("movq %%r15, %0": "=m"(e->jb.r15));
			asm volatile("movq %%rsp, %0": "=m"(e->jb.ssp));
			asm volatile("leaq (%%rip), %0;": "=r"(e->jb.rip));
			*/
			
			asm volatile("movq %%rbx, %0; movq %%rbp, %1; movq %%r12, %2; movq %%r13, %3; movq %%r14, %4; movq %%r15, %5; movq %%rsp, %6; leaq (%%rip), %7": 
						"=m"(e->jb.rbx), "=m"(e->jb.rbp), "=m"(e->jb.r12), "=m"(e->jb.r13), "=m"(e->jb.r14), "=m"(e->jb.r15), "=m"(e->jb.ssp), "=r"(e->jb.rip));
#else
			/* x86 Registers
			unsigned long ebx;
			unsigned long esi;
			...
			*/
			  ...
			  asm volatile("movl %%ebx, %0; movl %%esi, %1; movl %%edi, %2; movl %%ebp, %3; movl %%esp, %4; leal (%%eip), %5": 
							"=m"(e->jb.ebx), "=m"(e->jb.esi), "=m"(e->jb.edi), "=m"(e->jb.ebp), "=m"(e->jb.ssp), "=r"(e->jb.eip));
#endif

        }
        else if (!ml_mg::is_locked(o)) {
				//printf("%p: !ml_mg::is_locked(o) - try to extend\n", tx);
				//fflush(stdout);
...
/*-------------------------------------------------------------------------------------------------------*/
            ...
            int first_non_valid_index = try_to_extend_with_partial_rollback();
            
            gtm_rwlog_entry *first_rs_entry = tx->readlog.begin();
            gtm_rwlog_entry *last_rs_entry = tx->readlog.end();
            
            gtm_rwlog_entry *first_non_valid = tx->readlog[first_non_valid_index];
            
            if (first_non_valid != last_rs_entry) {
				if (first_non_valid == first_rs_entry)
					tx->restart(RESTART_VALIDATE_READ);
				else // first_non_valid is between first and last
					tx->restart(RESTART_VALIDATE_READ, false, first_non_valid_index);
			*/
            ...
            gtm_rwlog_entry *first_rs_entry = tx->readlog.begin();
            gtm_rwlog_entry *last_rs_entry = tx->readlog.end(); // index of entry next to the last one
            ...
            if (first_non_valid != last_rs_entry) {
				if (first_non_valid == first_rs_entry){
					...
					// first_non_valid is between first and last
					printf("%p: 500\n", tx);
					fflush(stdout);
  ...
  template <typename V> static V load(const V* addr, ls_modifier mod) {
	...
	printf("%p: reading address %p, value = %d, ts = %lld\n", tx, addr, (int)*((int*)addr), current_timestamp());
	fflush(stdout);
...
public:
  ...
  virtual bool trycommit(gtm_word& priv_time) {
    ...
    if (snapshot < ct - 1 && !validate(tx)){
		printf("%p: snapshot < ct = %d, !validate(tx) = %d\n", tx,snapshot < ct -1, !validate(tx));
		fflush(stdout);
      return false;
	}
