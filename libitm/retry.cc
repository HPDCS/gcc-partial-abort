  ...
  if (r == RESTART_INIT_METHOD_GROUP)
    {
      ...
      if ((state & STATE_SERIAL) == 0) {
		  ...
		  if (disp->get_method_group() == default_dispatch.load(memory_order_relaxed)->get_method_group())
...
parse_default_method()
{
  ...
  else if (strncmp(env, "pr_ml_wt", 8) == 0)
    {
      disp = GTM::dispatch_ml_wt_pr();
      env += 8;
    }
  else
    goto unknown;
 ...
 unknown:
  GTM::GTM_error("retry.cc: Unknown TM method in environment variable " 
                 "ITM_DEFAULT_METHOD\n");
  return 0;
}
